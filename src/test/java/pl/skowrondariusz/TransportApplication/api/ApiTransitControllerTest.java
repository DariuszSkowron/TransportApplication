...
//        LocalDate date3 = Calendar.getTime();
    ...
    public void shouldGetTransitFromId() throws Exception {
        ...
        when(transitService.getTransit(0L)).thenReturn(java.util.Optional.ofNullable(transitList.get(0)));

        this.mockMvc.perform(MockMvcRequestBuilders.get("/api/transits/{id}", 0L))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.sourceAdress", is("Poznań")));
//                .andExpect(jsonPath("$.distance", is("220")));

    }
//    transitList.get(1).getDistance().toString()))
//
//    @Test
//    public void shouldGetTransitFr
...
//
//
////        when(transitService.(0L)).thenReturn(java.util.Optional.ofNullable(transitList.get(0)));
////
